...
server <- function(input, output) {
  ...
  output$plotings<- renderPlot({
    ...
    if (input$Factor_Select == "Employment Type") {
      ...
      p <- ggplot(as.data.frame(tbl1), aes(x = reorder(employment_type, Freq), y = Freq, fill = fraudulent)) +
        ...
        geom_text(aes(label=Freq), position=position_dodge(width=0.9), vjust=-0.25, size=4)
      print(p)
  ...
  output$fraudulent_plotings<- renderPlot({


    if (input$frau_bal == "balanced") {
      ...
      name <- c("fraudulent", "unfraudulent")
      value <- c(fraudulent,unfraudulent)
    ...
    else if ( input$frau_bal == "unbalanced") {
      df <- read.csv("fake_job_postings.csv", header = T, stringsAsFactors = F, sep=",")
      fraudulent <- length(df$fraudulent[df$fraudulent==1])
      unfraudulent <-length(df$fraudulent[df$fraudulent==0])
      name <- c("fraudulent", "unfraudulent")
